...
const styles = {chip: {margin: 4,}, wrapper: {display: 'flex', flexWrap: 'wrap'},};
class ChipsContent extends Component {
    ...
    render() {
        var chips = this.props.attribute.enumerations.map((chip) => {
            return (<div key={chip}><Chip key={chip} style={styles}
                                          onRequestDelete={() => this.deleteEnumeration(chip)}>                         {chip}                     </Chip>
            </div>);
        });
